...
def robot_control_thread():
    ...
    while not shared_state["stop"]:
        with state_lock:
            ...
            dest = shared_state["dest"]

        if position is not None and orientation is not None:
            orien, dist = get_orien(dest, position)
            orien -= orientation
            ...
            print(orien)
            if orien > 10:
                set_motor_state(esp8266_base_url, "motorA", 10, "reverse")
                set_motor_state(esp8266_base_url, "motorB", 15, "forward")
                print("right")
                continue
            if orien < -10:
                set_motor_state(esp8266_base_url, "motorA", 15, "forward")
                set_motor_state(esp8266_base_url, "motorB", 10, "reverse")
                print("left")
                continue
            set_motor_state(esp8266_base_url, "motorA", 10, "forward")
            set_motor_state(esp8266_base_url, "motorB", 10, "forward")
